...
class DocumentProcessor:
    ...
    def process_documents(self):
        ...
        # Process EPUBs
        for epub_file in [f for f in os.listdir(self.script_dir) if f.lower().endswith('.epub')]:
            print(f"\nProcessing EPUB: {epub_file}")
            try:
                self._process_epub(epub_file)
            except Exception as e:
                print(f"✗ Failed: {str(e)}")

    def process_specific_chapter(self):
        # Get both PDF and EPUB files
        pdfs = [f for f in os.listdir(self.script_dir) if f.lower().endswith('.pdf')]
        epubs = [f for f in os.listdir(self.script_dir) if f.lower().endswith('.epub')]
        documents = pdfs + epubs
        
        if not documents:
            print("No PDFs or EPUBs found in directory!")
            return
        ...
        try:
            doc_choice = int(input("Select document (number): ")) - 1
            selected_doc = documents[doc_choice]
        ...
        print(f"\nProcessing chapter {chapter_num} from {selected_doc}")
        try:
            if selected_doc.lower().endswith('.pdf'):
                self._process_pdf(selected_doc, specific_chapter=chapter_num)
            else:
                self._process_epub(selected_doc, specific_chapter=chapter_num)
        except Exception as e:
            print(f"✗ Failed: {str(e)}")

    def _process_epub(self, filename, specific_chapter=None):
        """Process an EPUB file and convert it to images for Nokia 5310"""
        book = epub.read_epub(os.path.join(self.script_dir, filename))
        ...
        # If processing specific chapter, find it
        if specific_chapter:
            chapter_found = False
            for i, (chapter_name, content) in enumerate(chapters):
                ...
                if any(num.lstrip('0') == specific_chapter.lstrip('0') for num in chap_numbers):
                    ...
                    self._process_epub_chapter(content, chapter_name, base_dir, i)
                    chapter_found = True
                    break
            
            if not chapter_found:
                print(f"Chapter {specific_chapter} not found in EPUB!")
            return

        # Process all chapters
        max_chapters = self.test_page_limit if self.test_mode else len(chapters)
        for i, (chapter_name, content) in enumerate(chapters[:max_chapters]):
            try:
                self._process_epub_chapter(content, chapter_name, base_dir, i)
            except Exception as e:
                print(f"Chapter {chapter_name} error: {str(e)}")

    def _extract_epub_chapters(self, book):
    ...
    def _process_epub_chapter(self, content, chapter_name, base_dir, chapter_index):
        """Process a single EPUB chapter and convert to images"""
        # Create chapter folder
        safe_chapter_name = self._sanitize(chapter_name)
        chapter_dir = os.path.join(base_dir, f"{chapter_index:04d}_{safe_chapter_name}")
        os.makedirs(chapter_dir, exist_ok=True)
        
        print(f"Processing chapter: {chapter_name} to {chapter_dir}")
        
        # Convert text to images
        self._text_to_images(content, chapter_dir, chapter_name)

    def _text_to_images(self, text, output_dir, chapter_name):
        """Convert text content to images formatted for Nokia 5310"""
        ...
        for word in words:
            if char_count + len(word) + 1 > 1000:  # +1 for space
        ...
        for i, page_text in enumerate(pages):
            img = self._render_text_to_image(page_text, chapter_name if i == 0 else "")
            if img:
                # Split into top and bottom parts like the PDF processor
                width, height = img.size
                
                top_split = img.crop((0, 0, width, int(height * 0.55)))
                bottom_split = img.crop((0, int(height * 0.45), width, height))
                
                # Save both parts as BMP files
                if np.array(top_split.convert("L")).min() < 240:
                    output_path = os.path.join(output_dir, f"{i+1:04d}a.bmp")
                    top_split.convert("1", dither=Image.FLOYDSTEINBERG).transpose(Image.ROTATE_270).save(output_path)
                
                if np.array(bottom_split.convert("L")).min() < 240:
                    output_path = os.path.join(output_dir, f"{i+1:04d}b.bmp")
                    bottom_split.convert("1", dither=Image.FLOYDSTEINBERG).transpose(Image.ROTATE_270).save(output_path)

    def _render_text_to_image(self, text, heading=""):
        """Render text to image with proper formatting for Nokia 5310"""
        # Use a fixed width that matches the PDF processor output after rotation
        # The PDF processor creates images that are rotated 270 degrees, so we need to create
        # images that will be the correct orientation after rotation
        target_width = 320  # Height after rotation
        target_height = 240  # Width after rotation
        
        # Create a temporary image to calculate text layout
        ...
        for word in words:
            ...
            # Estimate text width
            text_width = len(test_line) * 7 if font is None else font.getlength(test_line)
            
            if text_width < target_width - 20:  # 20px margin
        ...
        required_height = y_position + len(lines) * line_height + 10
        
        # Create final image with calculated height
        img = Image.new('L', (target_width, required_height), 255)
        draw = ImageDraw.Draw(img)
        
        # Add heading if provided
        y_position = 10
        if heading and heading_font:
            draw.text((10, y_position), heading, fill=0, font=heading_font)
            y_position += line_height
        
        # Render lines
        for line in lines:
            draw.text((10, y_position), line, fill=0, font=font)
            y_position += line_height
